...
on devrais parcourir toute la move_queue ou display_queue pour trouver l'avion.
...
Pour:
    ...
    plus maintenable
Contre:
    plus compliqué as implémenté
    compilation plus grosse


rôle
    Pour:
        rapide as implémenté
    Contre:
        classe plus lourde
        moins maintenable
        Single-Responsability
...
la fonction timer dans opengl_interface
...
opengl_interface dans move_queue

3. Comment fait-on pour supprimer la référence sur un avion qui va être détruit dans ces deux structures ?
on parcourt la move_queue avec un iterateurs et on supprime l'avion lorsque necessaire.
...
il faut définir context_initializer avant de définir la factory. comme cela avec la liste des initialisateurs on sera obliger d'initialiser le context avant la factory.
